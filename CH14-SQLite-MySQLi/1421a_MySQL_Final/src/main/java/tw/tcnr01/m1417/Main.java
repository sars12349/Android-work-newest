...
public class Main extends AppCompatActivity implements View.OnClickListener , AdapterView.OnItemSelectedListener  {
    ...
    private LinearLayout blinear01, blinear02;
    ...
    String  t_id, taddress;

    // ----------定時更新--------------------------------
    private Long startTime;
    private Handler handler = new Handler();

    int autotime = 2;// 要幾秒的時間 更新匯入MySQL資料
    // ------------------
...
//        宣告鈴聲
        ToneGenerator tongG=new ToneGenerator(AudioManager.STREAM_ALARM,100); //100=max
        tongG.startTone(ToneGenerator.TONE_CDMA_ALERT_NETWORK_LITE,1000);
        tongG.release();
    }
...
//            mSpnName.setSelection(old_index, true); // spinner 小窗跳到第幾筆
...
//                mSpnName.setEnabled(true);
    ...
    private void mysql_insert() {
        ArrayList<NameValuePair> nameValueParis=new ArrayList<NameValuePair>();

        nameValueParis.add(new BasicNameValuePair("name",tname));
        nameValueParis.add(new BasicNameValuePair("grp",tgrp));
        nameValueParis.add(new BasicNameValuePair("address",taddr));
        //-----------------------------
        String result=DBConnector.executeInsert("SELECT * FROM member",nameValueParis);
...
//        mSpnName.setOnItemSelectedListener(mSpnNameOnItemSelLis);
    ...
    private void showRec(int index) {
        ...
        if (recSet.size() != 0) {
            ...
            e003.setText(fld[3]);
            mSpnName.setSelection(index, true); //spinner 小窗跳到第幾筆
//            mSpnName.setSelection(show_index, true); //spinner 小窗跳到第幾筆
//            u_spin
        } else {
...
//                mSpnName.setVisibility(View.VISIBLE);
    ...
    //--------------------------------------------------------------------------------------
    private void dbmysql() {     // 讀取MySQL 資料
        mContRes = getContentResolver();
